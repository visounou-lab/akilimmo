      ...
      >
        ...
        {success ? (
          ...
          <form onSubmit={submit} className="mt-7 space-y-5">
            <fieldset>
              <legend className="mb-2 text-sm font-semibold text-[#3D3530]">Motif *</legend>
              <div className="grid gap-2" role="radiogroup">
                {reasons.map(([value, label]) => (
                  <label
                    key={value}
                    className={`flex min-h-11 cursor-pointer items-center gap-3 rounded-xl border bg-white px-4 py-3 text-sm transition-colors ${
                      selectedReason === value
                        ? "border-[#1B4D3E] text-[#1B4D3E] ring-2 ring-[#1B4D3E]/15"
                        : "border-[#DCCDBD] text-[#3D3530] hover:border-[#A98D72]"
                    }`}
                  >
                    <input
                      type="radio"
            ...
            <label className="block">
              ...
              <textarea name="details" required minLength={10} maxLength={1500} rows={4} className="relative block w-full rounded-xl border border-[#DCCDBD] bg-[#FFFFFF] px-4 py-3 text-base text-[#1C1917] shadow-sm focus:border-[#1B4D3E] focus:outline-none focus:ring-2 focus:ring-[#1B4D3E]/20" />
            </label>
            <label className="block">
