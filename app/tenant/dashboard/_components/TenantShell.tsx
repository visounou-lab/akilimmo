...
const PAGE_TITLES: Record<string, string> = {
  ...
  "/tenant/dashboard/contrat":   "Mon contrat",
  "/tenant/dashboard/paiements": "Mes paiements",
  "/tenant/dashboard/profil":    "Mon profil",
};

function getPageTitle(pathname: string): string {
  if (PAGE_TITLES[pathname]) return PAGE_TITLES[pathname];
  const match = Object.entries(PAGE_TITLES)
    .filter(([key]) => key !== "/tenant/dashboard" && pathname.startsWith(key))
    .sort((a, b) => b[0].length - a[0].length)[0];
  return match?.[1] ?? "Mon espace";
}

export default function TenantShell({ children, userName, userEmail, userInitial, signOutAction }: Props) {
  ...
  return (
    <div className="flex min-h-screen bg-gray-50">
      ...
      {sidebarOpen && (
        <div className="fixed inset-0 bg-black/50 z-40 md:hidden" onClick={() => setSidebarOpen(false)} aria-hidden="true" />
      )}
      ...
      <aside className={[
        ...
        <div className="flex-1 overflow-y-auto py-4 px-2 lg:px-3">
          <TenantSidebarNav onClose={() => setSidebarOpen(false)} />
        </div>
